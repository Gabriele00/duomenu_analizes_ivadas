...
* nubraižykite stulpelinę diagramą kiekvienam pardarvėjui
...
## mapply
* `mapply` taiko paraleliai (vienu metu) funckiją skirtingiems argumentams arba list arba vektoriams
* m - multivariate
```{r}
str(mapply)
```
...
## mapply
Exercise
Generate following matrix using `mapply` (hint: use `rep()`)
```{r, echo=FALSE}
mapply(rep, 1:4,4)
```
...
aggregate(airquality, 
          list(airquality$Month), #jeigu nebus list() mes klaid, nes tada bus vektorius
          mean, na.rm=TRUE)
